import { mostrarToast } from "../funcionesGaleriaIndumentaria/funcionesGaleria.js";

/*
    Setea los eventos para cerrar y abrir la pantalla donde se va a mostrar el carrito
*/
function setearEstadoCarrito(carrito,arrayIndumentaria){
    ...
    agregarEventoVaciarCarrito(botonVaciarCarrito,carrito,arrayIndumentaria);
    let botonFinalizarCompra=document.getElementById("finalizar__compra");
    agregarEventoFinalizarCompra(botonFinalizarCompra,carrito,arrayIndumentaria);
}
/*
    Agrega al boton que vacia el carrito, su respectivo evento para cumplir con esta funcionalidad.
 */
function agregarEventoVaciarCarrito(botonVaciarCarrito,carrito,arrayIndumentaria){
        ...
        if(carrito.length>0){
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    resetearBotonesCompra(arrayIndumentaria);
                    carrito.length=0;
...
*/
function agregarEventoFinalizarCompra(botonFinalizarCompra,carrito,arrayIndumentaria){
...
function crearProductoEnCarrito(producto,carrito){
    ...
    botonEliminar.onclick=()=>{
        let indiceBuscado=carrito.findIndex(prenda=>prenda.nombre==producto.nombre);
...
function resetearBotonesCompra(arrayIndumentaria){
    for (let prenda of arrayIndumentaria){
        let boton=document.getElementById(`${prenda.id}_boton`);
        boton.innerText="AGREGAR AL CARRITO";
        document.getElementById((prenda.id)).append(boton);
    }
}
/*
    Actualiza el boton de un prenda, de "AGREGAR AL CARRITO" a "EN EL CARRITO".
*/
function actualizarBotonCompra(prendaSeleccionado,carrito){
    let idBuscado=`${prendaSeleccionado.id}_boton`;
    let botonBuscado=document.getElementById(idBuscado);
    if(carrito.some(prenda=>prendaSeleccionado.nombre==prenda.nombre)){
